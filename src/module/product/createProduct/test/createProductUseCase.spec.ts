...
describe("US-1 : Créer un produit", () => {
  ...
  test("Scénario 6 : création échouée, description trop courte", async () => {
    //Étant donné qu'il n'y a pas de produit enregistré
    const createProductRepository = new CreateProductMockFailRepository();
    const createProductUseCase = new CreateProductUseCase(
      createProductRepository
    ...
    await expect(
      ...
      // Alors une erreur doit être envoyée «la description doit faire au moins 6 caractères»
    ).rejects.toThrow("la description doit faire au moins 6 caractères");
  });
});
